...
**Goal:** Implement a lightweight CNN with **< 1 Million parameters** to establish a performance baseline [cite: 27-28].
* **Architecture:** Basic stack of `Conv2d` -> `ReLU` -> `MaxPool` layers.
* **Deliverables:** Accuracy curves, confusion matrix, and parameter count analysis [cite: 29-33].
* **Theory Checkpoint:** Investigation of representational power by removing non-linearities (*Linear Collapse Experiment*) [cite: 34-38].

### Stage 2: Architecture Scaling & Diagnostics
**Goal:** Analyze the impact of scaling network dimensions independently [cite: 40-41].
* **Experiments:**
...
* **Diagnostics:** Analysis of overfitting behavior and optimization stability [cite: 45-52].
...
**Goal:** Stabilize training and mitigate overfitting using modern deep learning techniques [cite: 53-59].
...
**Goal:** Rigorous reproduction of a standard industry benchmark from scratch [cite: 73-76].
* **Model:** **ResNet-18** (adapted for CIFAR-10 input size).
* **Training Recipe:** SGD with Momentum (0.9) and **Cosine Annealing Scheduler** for 200 epochs [cite: 82-86].
* **Target:** Achieve >93% accuracy (within 3.0 points of literature) [cite: 88-89].

### Stage 5: Surpass the SOTA Model
**Goal:** Implement architectural and training enhancements to beat the Stage 4 baseline [cite: 96-98].
...
* **Ablation Studies:** Systematic removal of components (No-MixUp, No-SE, No-Cutout) to quantify individual contributions [cite: 133-137].
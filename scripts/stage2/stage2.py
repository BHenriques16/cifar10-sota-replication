...
class CNN_Width(nn.Module):
    def __init__(self):
        ...
        # Linear Input: 256 channels * 4 * 4 spatial size = 4096
        self.fc = nn.Sequential(
            nn.Linear(4096, 512), # Increased hidden layer too
            nn.ReLU(),
            nn.Linear(512, 10) 
        )
...
class CNN_Depth(nn.Module):
    def __init__(self):
        ...
        # NEW Block 3 (Extra Depth - No Pooling)
        self.extra_block1 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.ReLU()
        )

        # NEW Block 4 (Extra Depth - No Pooling)
        self.extra_block2 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.ReLU()
        )
        
        # Block 5 (Original Block 3)
        self.block3 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, padding=1),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # Pass through new extra blocks
        x = self.extra_block1(x)
        x = self.extra_block2(x)
...
def main():
    ...
    model_width = CNN_Width().to(device)
    summary(model_width, input_size=(3, 32, 32))
    
    criterion = nn.CrossEntropyLoss()
    optimizer_width = optim.Adam(model_width.parameters(), lr=0.001)
